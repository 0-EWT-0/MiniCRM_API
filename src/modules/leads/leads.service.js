...
class LeadService {
    ...
    `;
            ...
            const userRes = yield this.db.query(`SELECT email, name FROM users_crm WHERE user_id = $1`, [lead.id_user_id]);
            const user = userRes.rows[0];
            if (!user) {
                throw new Error("Usuario no encontrado para enviar el correo.");
            }
            try {
                yield emailjs.send(process.env.EMAILJS_SERVICE_ID, process.env.EMAILJS_TEMPLATE_ID, {
